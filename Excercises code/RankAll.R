rankall <- function(outcome, num = "best") {
  ...
  rank_hospital <- function(state_data1, num) {
    ordered_state_data <- order(state_data1[3], state_data1$Hospital.Name, na.last=NA)
    
    if (num == "best") {
      state_data1$Hospital.Name[ordered_state_data[1]]
    } else if (num == "worst") {
      state_data1$Hospital.Name[ordered_state_data[length(ordered_state_data)]]
    } else if (is.numeric(num)) {
      state_data1$Hospital.Name[ordered_state_data[num]]
    } else {
      stop("invalid num")
    }
  }
